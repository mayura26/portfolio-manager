...
export async function TopTradesTable({ type, since, sector }: Props) {
  ...
  const filtered = clusters
    ...
    .sort((a, b) =>
      type === "buy" ? b.buyCount - a.buyCount : b.sellCount - a.sellCount,
    )
    .slice(0, 10);
    ...
    <div className="hairline bg-surface">
      <div className="border-b border-border px-5 py-3">
        ...
        <p className="mt-0.5 text-xs text-muted">
          {type === "buy" ? "Most congress members buying" : "Most congress members selling"}
        </p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="px-5 py-2 text-left text-xs text-muted font-normal">Ticker</th>
              <th className="px-5 py-2 text-left text-xs text-muted font-normal">Sector</th>
              <th className="px-5 py-2 text-right text-xs text-muted font-normal">Members</th>
              <th className="hidden px-5 py-2 text-right text-xs text-muted font-normal sm:table-cell">Latest</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((c) => {
              ...
              const extra = c.politicians.length > 3 ? ` +${c.politicians.length - 3}` : "";
              return (
                <tr key={c.ticker} className="border-b border-border last:border-0 hover:bg-surface-elevated">
                  ...
                  <td className="px-5 py-2.5 text-right">
                    <span className={["tabular text-sm font-medium", badgeClass].join(" ")}>
                      {count}
                    </span>
                  </td>
