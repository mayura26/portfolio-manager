...
import type { GroupPortfolioPerformance } from "@/lib/holding-performance";

type Props = {
  performance: GroupPortfolioPerformance;
};

export function GroupPerformanceTable({ performance }: Props) {
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              ...
              >
                No portfolio holdings to measure yet.
              </td>
            </tr>
          ) : (
            rows.map((row) => (
